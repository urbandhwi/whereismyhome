...
# streamlit_folium 설치 (Streamlit 앱이 실행될 환경에서 이 명령이 필요할 수 있습니다)
# st.write('Installing streamlit_folium if not present...')
# try:
#     import streamlit_folium
# except ImportError:
#     import subprocess
#     subprocess.check_call(['pip', 'install', 'streamlit-folium'])
#     import streamlit_folium
...
def load_data(base_url):
    ...
    try:
        ...
        # Ensure 'LINE_NUM' is a string for Folium tooltip to avoid potential KeyError
        if 'LINE_NUM' in geojson_subway.columns:
            geojson_subway['LINE_NUM'] = geojson_subway['LINE_NUM'].astype(str).fillna('')
